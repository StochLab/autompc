...
class HalfcheetahCost(Cost):
    ...
    def eval_obs_cost(self, obs):
        ...
        # # speed reward
        # reward_velocity = 10*qvel[0]

        # # height velocity reward
        # reward_height = 0*-(0 - qvel[1])

        # # angle reward
        # reward_angle = -100*(0 - qpos[2])**2 -10*(0 - qvel[2])**2

        # return 100 - (reward_velocity + reward_height + reward_angle)

        # speed cost
        cost_speed = -10*qvel[0]
        # angle cost
        cost_angle = 100*(0 - qpos[2])**2 + 10*(0 - qvel[2])**2
        return cost_speed + cost_angle
    ...
    def eval_ctrl_cost(self, ctrl):
        # return 100 -0.05 * np.square(ctrl).sum()
        return 0.1 * np.square(ctrl).sum()
